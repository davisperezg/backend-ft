...
import { fileExists, formatListInvoices } from 'src/lib/functions';
...
@Injectable()
export class InvoicePendingJobs {
  private logger = new Logger('InvoicePendingJobs');
  ...
  constructor(
    ...
    @InjectQueue('job-sunat-submission-pending')
  ...
  async handleCron() {
    this.logger.debug(
      'INICIANDO ENVIO DE COMPROBANTES A SUNAT PENDIENTES MEDIANTE CRON...',
    );
    try {
        ...
        if (item.establishmentHasJob) {
          ...
          for (let indexIvs = 0; indexIvs < invoices.length; indexIvs++) {
            ...
            if (
              ...
              const updated = await this.invoiceService.updateInvoiceFields(
                invoice.id,
                { estado_operacion: 0 },
              ); //cambiamos a creado
              ...
              //Notificamos al cliente que se acepto a sunat
              this.invoiceGateway.server
                .to(`room_invoices_emp-${companyId}_est-${establishmentId}`)
                .emit('server::newInvoice', _invoice);
            }
          }
        }
      ...
      this.logger.error(
        'OCURRIO UN ERROR EN EL PROCESO DE CRON PENDIENTES.',
        e.message,
      );
    } finally {
      this.logger.debug(
        'PROCESO DE ENVIO A SUNAT PENDIENTES MEDIANTE CRON A FINALIZADO.',
      );
    }
  }
}
