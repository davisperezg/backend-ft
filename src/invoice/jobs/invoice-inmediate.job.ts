...
@Injectable()
export class InvoiceInmediateJobs {
  private logger = new Logger('InvoiceInmediateJobs');
  ...
  constructor(
    ...
    @InjectQueue('job-sunat-submission-inmediate')
  ...
  async handleCron() {
    ...
    try {
        ...
        if (item.establishmentHasJob) {
          ...
          for (let indexIvs = 0; indexIvs < invoices.length; indexIvs++) {
            ...
            if (
                ...
                try {
                  ...
                  await this.sunatQueue.add(
                    'submitSunatInmediate',
                    {
                      ...
                      sendModeDoc: EnvioSunatModo.INMEDIATO,
              ...
              await this.sunatQueue.add(
                'submitSunatInmediateWithValidation',
                {
                  invoiceId: invoice.id,
                  roomId: roomId,
                  sendMode: establishmentModeSend,
      ...
      this.logger.error(
        'OCURRIO UN ERROR EN EL PROCESO DE CRON INMEDIATO.',
        e.message,
      );
    } finally {
      this.logger.debug(
        'PROCESO DE ENVIO A SUNAT INMEDIATO MEDIANTE CRON A FINALIZADO.',
      );
    }
  }
}
