...
@Injectable()
export class InvoiceManualJobs {
  private logger = new Logger('InvoiceManualJobs');
  ...
  constructor(
    ...
    @InjectQueue('job-sunat-submission-manual')
  ...
  async handleCron() {
    this.logger.debug(
      'INICIANDO ENVIO DE COMPROBANTES A SUNAT TIPO MANUAL MEDIANTE CRON...',
    );
    try {
        ...
        if (item.establishmentHasJob) {
          ...
          for (let indexIvs = 0; indexIvs < invoices.length; indexIvs++) {
            ...
            if (
                ...
                try {
                  ...
                  await this.sunatQueue.add(
                    'submitSunatManual',
                    {
                      ...
                      sendModeDoc: EnvioSunatModo.MANUAL,
              ...
              await this.sunatQueue.add(
                'submitSunatManualWithValidation',
                {
                  invoiceId: invoice.id,
                  roomId: roomId,
                  sendMode: establishmentModeSend,
      ...
      this.logger.error(
        'OCURRIO UN ERROR EN EL PROCESO DE CRON MANUAL.',
        e.message,
      );
    } finally {
      this.logger.debug(
        'PROCESO DE ENVIO A SUNAT TIPO MANUAL MEDIANTE CRON A FINALIZADO.',
      );
    }
  }
}
