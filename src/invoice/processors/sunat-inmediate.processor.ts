...
@Processor('job-sunat-submission-inmediate')
export class SunatInmediateProcessor {
  private readonly logger = new Logger(SunatInmediateProcessor.name);
  ...
  async handleSunatSubmission(
    ...
    this.logger.log(
      `Procesando envío a SUNAT INMEDIATO para factura ID: ${invoiceId}`,
    );
    ...
    try {
      ...
      if (!invoice) {
        throw new Error(
          `No se encontró la factura con ID: ${invoiceId} - INMEDIATO`,
        );
      }
      ...
      this.logger.log(
        `Factura ID: ${invoiceId} procesada correctamente en SUNAT INMEDIATO`,
      );

      return sunatResponse;
    } catch (error) {
      this.logger.error(
        `Error procesando envío a SUNAT INMEDIATO: ${error.message}`,
        error.stack,
      );
  ...
  @Process('submitSunatInmediateWithValidation')
